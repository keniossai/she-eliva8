                        ...
                        <div class="p-4 border-top">
                            <form method="POST" action="{{ route('post.update', $post->id) }}" enctype="multipart/form-data">
                                                ...
                                                @foreach ($categories as $category)
                                                    <option
                                                        @foreach ($post->categories as $postCastegory)
                                                            {{ $postCastegory->id == $category->id ? 'selected' : '' }}
                                                        @endforeach
                                ...
                                <div class="mb-0">
                                    ...
                                    <textarea name="body" id="body" placeholder="Enter post description">{{ $post->body }}</textarea>
                                </div>
                                <div class="mb-0">
                                    <div class="form-check form-switch form-switch-md">
                                        <input type="checkbox" class="form-check-input" id="publish" name="status" value="1" {{ $post->status == true ? 'checked' : '' }}>
